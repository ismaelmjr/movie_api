...
let allowedOrigins = [
  ...
  "http://testsite.com",
  "http://localhost:1234",
  "https://topimdbmovies.netlify.app",
];

app.use(
  cors({
    // function for sharing resources to certain domains otherwise displaying an error message.
...
app.post(
  ...
  (req, res) => {
    ...
    Users.findOne({ Username: req.body.Username }) // check to see if a user already exist.
      .then((user) => {
        if (user) {
          ...
          Users.create({
            ...
            }) // Within this callback, you then send a response back to the client that contains both a status code and the document (called “user”).
            .catch((error) => {
              console.error(error);
      ...
      .catch((error) => {
        // error-handler if anything goes wrong with the HTTP POST.
        console.error(error);
